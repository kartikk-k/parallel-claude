...
class TerminalManager {
  ...
  createTerminal(
    ...
    if (this.terminals.has(sessionId)) {
      ...
      console.log(`[TerminalManager] Terminal ${sessionId} already exists, reattaching`);
      this.attachTerminal(sessionId, container);
      return;
    }
    ...
    try {
      ...
      webglAddon.onContextLoss(() => {
        console.warn(`[TerminalManager] WebGL context lost for ${sessionId}`);
      });
      xterm.loadAddon(webglAddon);
      console.log(`[TerminalManager] WebGL enabled for ${sessionId}`);
    } catch (err) {
      console.log(`[TerminalManager] Canvas renderer for ${sessionId}`);
    }
    ...
    // Setup IPC handlers
    this.setupIPCHandlers(sessionId, repositoryId, xterm, onStatusUpdate);
  }
  ...
  private setupIPCHandlers(
    ...
    const unsubscribeData = window.electron?.ipcRenderer.on(
      ...
      (...args: unknown[]) => {
        ...
        if (receivedSessionId === sessionId) {
          ...
          // Status update can be handled by consumer if needed
          // We're keeping TerminalManager pure and not importing parseTerminalStatusDebounced
        }
      }
    );
    ...
    this.dataSubscriptions.set(sessionId, () => {
